...
- **Graph** — the company knowledge graph: explore from any company (one-hop ego graph + on-demand neighbour expansion) and save/load explored subgraphs per company. Read-only research view, lazy-loaded so the graph library stays out of the main bundle.
